...
def parse_date(value):
    ...
    try:
        raw = str(value).strip()
        dayfirst = "-" in raw and "/" not in raw
        return dateparser.parse(raw, dayfirst=dayfirst, fuzzy=True)
...
def parse_budget(value):
    ...
    if "-" in s:
        ...
        has_k = any("k" in p for p in parts)
        nums = []
        for p in parts:
            mult = 1000 if ("k" in p or has_k) else 1
            try:
                nums.append(float(p.replace("k", "")) * mult)
            except ValueError:
                pass
        return sum(nums) / len(nums) if nums else None
...
def load_and_score(csv_path_or_buffer):
    # Streamlit keeps the uploaded file object alive across reruns, and pandas
    # leaves the read pointer at EOF. Without this, the second rerun (any widget
    # click) reads an empty buffer and raises EmptyDataError.
    ...
    valid_email = df["email"].str.strip().ne("") & df["email"].str.contains(
        r"^[^@\s]+@[^@\s.]+\.[^@\s]+$", regex=True)
    df = pd.concat([df[~valid_email],
                    df[valid_email].drop_duplicates(subset=["email"], keep="first")])
